...
function schema(extra = []) {
  return JSON.stringify({ "@context": "https://schema.org", "@graph": [
    ...
    { "@type": ["SoftwareApplication", "MobileApplication"], "@id": `${site.url}/#app`, name: "Bona Fide: SIE Exam Prep", applicationCategory: "EducationalApplication", operatingSystem: "iOS", description: "SIE exam practice questions, readiness score, timed mock exam and contextual tutor.", url: site.url, installUrl: `https://apps.apple.com/app/id${site.appStoreId}`, sameAs: `https://apps.apple.com/app/id${site.appStoreId}`, isAccessibleForFree: true, featureList: ["1,600 SIE practice questions", "Readiness score across four FINRA domains", "75-question, 105-minute mock exam", "Tutor explanations for missed questions"], offers: { "@type": "Offer", price: "0", priceCurrency: "USD", description: "Free diagnostic and daily practice. A Pro subscription unlocks unlimited questions, mock exams and tutor access.", availability: "https://schema.org/InStock" }, publisher: { "@id": `${site.url}/#organization` } },
    ...extra,
  ] });
}
...
const footer = `<footer><p>© 2026 Bona Fide. Independent SIE exam preparation.</p><p><a href="${official}">FINRA SIE exam information</a> · <a href="${site.legalBase}/privacy/">Privacy</a> · <a href="${site.legalBase}/terms/">Terms</a> · <a href="/support/">Support</a></p></footer>`;
const storeCta = `<a class="button" href="${appUrl}"${site.appStoreUrl ? "" : ' aria-disabled="true"'}>Download on the App Store</a>`;
const faqHtml = faqs.map(([q,a]) => `<details><summary>${esc(q)}</summary><p>${esc(a)}</p><a href="/guides/">Read the study guides.</a></details>`).join("");
